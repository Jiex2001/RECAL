...
def tokenize(name: str):
    """Split on / \\ . _ - : space, lowercase, drop pure-digit tokens."""
...
def transition_profile(src, dst, rel, ts_nanos, num_nodes, n_rel,
    ...
    weight of every *consecutive* pair inside the buffer as a continuous bigram

        P_v[r(e_{i-1}), r(e_i)] += exp(-lam * (t_now - t(e_i)))

    with t_now the node's own last event time, and time in seconds.
