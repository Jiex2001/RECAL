# Adapted from MAGIC (https://github.com/FDUDSDE/MAGIC),
# Copyright (c) 2023 Jimmyokok, used under the MIT License.
...
# Dataset file split, copied verbatim from MAGIC/utils/trace_parser.py.
metadata = {
    'trace': {
...
# Names carried by Event records.
...
def _dig_name(rec_type, body):
    """Pull the display name out of one decoded non-Event CDM record."""
...
def parse_volume(dataset, file, id_type, id_name, node_types, relations,
    ...
    Mirrors MAGIC read_single_graph: reverse READ/RECV/LOAD, sort by
    timestamp, number nodes in time order, then dedup -- by (u, v, r) for us.
    Event records are also mined for entity names (pass-2 backfill).
    """
    path = os.path.join(HERE, 'data', 'dataset', dataset, file)
    ...
    with open(path, 'r', encoding='utf-8') as f:
        for line in f:
            ...
            m = pattern_event_exec.search(line)          # cadets process names
...
if __name__ == '__main__':
    ...
    ap.add_argument('--magic_label_filter', action='store_true',
                    help='restore the MAGIC label filter (drop malicious nodes from train graphs)')
    a = ap.parse_args()
