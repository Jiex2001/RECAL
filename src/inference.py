...
`{node: {relation: error}}` dict, which is not representable at 3.4M test
...
def compute_errors(model, g, n_rounds=10, seed=0):
    """Reconstruction errors of one graph as an ErrorTable."""
    model.eval()
    nodes, rels, errs = [], [], []
...
def compute_embeddings(model, g, sample=None, seed=0):
    """Node embeddings, optionally subsampled to `sample` rows."""
    model.eval()
    rep = model.embed(g)
