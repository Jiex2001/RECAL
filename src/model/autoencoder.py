# Derived from MAGIC (https://github.com/FDUDSDE/MAGIC),
# Copyright (c) 2023 Jimmyokok, used under the MIT License.
"""RECAL model, structure from MAGIC/model/autoencoder.py.
...
Differences from upstream, all required by the document:
  ...
  * `embed()` returns the three encoder layers concatenated and passed through
    `encoder_to_decoder` (upstream returns the last layer
    only, which is inconsistent with its own decoder input).
  * link prediction is off by default; when on, the term matches
...
class RECALModel(nn.Module):
    ...
    def embed(self, g):
        """Node embedding for KNN and the decoder heads."""
        return self._encode(g, None)[1]

    def node_errors(self, g, mask_bool):
        """Per-relation reconstruction error of the masked nodes.
    ...
    def _link_pred_loss(self, g, enc_rep):
        """MAGIC's Structural Reconstruction term."""
