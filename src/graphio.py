"""Graph + feature loading, per-relation edge caches, calibration split.
...
Never touches `malicious.pkl`; test labels are loaded by `src/metrics.py` only.
"""
...
def split_tags(meta, calib_index=-1):
    """(train tags, calib tag, test tags); calib is the last training graph."""
    n_tr = meta['n_train']
    c = calib_index % n_tr
...
def relation_freq(dataset, train_tags):
    ...
    The mask rates of Eq.3 call for training-set event counts; the calibration
    graph is excluded from training, so it is excluded here too.
    """
...
def feature_columns(dataset, use_semantic=True, use_profile=True):
    """Column indices of the Eq.1 blocks enabled by the semantic/profile flags."""
...
class Graph:
    ...
    def __init__(self, tag, num_nodes, x, edge_index, etype, edge_attr, ntype,
        ...
        self.edge_attr = edge_attr       # (E, R) float32 one-hot
...
def _rel_caches(edge_index, etype, n_rel, num_nodes):
    """Per relation the symmetrized, deduplicated edge set + participants.
